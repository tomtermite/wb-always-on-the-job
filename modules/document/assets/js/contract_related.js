  ...
  $(document).on("dblclick","#document-advanced .tr-pointer",function() {
    ...
    if(type == "file"){
      if(share == false){
        ...
        $.get(admin_url + 'document/get_hash_related/' + related_id + '/' + pathname + '/' + id_set).done(function(response) {
          response = JSON.parse(response);
          window.location.replace(site_url + 'document/document_client/file_view_share_related/'+response.hash);
        })
      }
    }else{
