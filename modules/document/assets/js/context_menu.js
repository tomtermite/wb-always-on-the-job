...
function toggleMenuOn() {
  ...
  if (menuState !== 1) {
    ...
    if (type == "file") {
        ...
        if (role == 1) {
          ...
          length_edit == 0
            ? ($(".context-menu__items").append(
                '<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="edit"><i class="fa fa-cog"></i> ' +
                  edit +
                  " </a></li>"
              ),
              $(".context-menu__items").append(
                '<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="d_file"><i class="fa fa-download"></i> ' +
                  download_file +
                  "</a></li>"
              ))
            : ($('.context-menu__link[data-action="edit"]').parent().remove(),
      ...
      var length_download = $(
        '.context-menu__link[data-action="d_file"]'
      ).length;
      length_download == 0
        ? ""
        : $('.context-menu__link[data-action="d_file"]').parent().remove();
      ...
      if (share == false) {
        length == 0
          ? ($(".context-menu__items").append(
              '<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="create_file"><i class="fa fa-plus"></i> ' +
                create_file +
                "</a>"
            ),
            $(".context-menu__items").append(
              '<li class="context-menu__item"><a href="#" class="context-menu__link" data-action="create_folder"><i class="fa fa-plus"></i> ' +
                create_folder +
                "</a>"
            ))
...
function menuItemListener(link) {
  ...
  switch (moveToAlbumSelectedId) {
    case "edit":
      if (type == "file") {
        ...
        if (share == false) {
          ...
          requestGet(admin_url + "document/check_file_exits/" + id_set).done(
            function (response) {
              response = JSON.parse(response);
              if (response.success) {
                requestGet(
                  admin_url + "document/get_hash_staff/" + id_set
                ).done(function (response1) {
                  response1 = JSON.parse(response1);
                  window.location.replace(
                    admin_url + "document/file_view_share/" + response1.hash
                  );
                });
              } else {
                alert_float("warning", response.message);
        ...
        if (share == true) {
          var length_submot_share = $(
            '#AddFolderModal .modal-footer [type="submit"]'
          ).length;
          length_submot_share == 0
            ? $("#AddFolderModal .modal-footer").append(
      ...
      break;
    case "delete":
      $.post(admin_url + "document/delete_folder_file/" + id_set).done(
        function (response) {
          response = JSON.parse(response);
          ...
          function (response) {
            ...
            if (response.group_share_client == 2) {
              ...
              if (response.client_groups_share || response.clients_share) {
                var client_groups_share =
                  response.client_groups_share.split(",");
    ...
    case "related":
      ...
      if (val != "") {
        requestGet(admin_url + "document/get_related_id/" + val).done(function (
          response_s
        ) {
    ...
    case "d_zip":
      ...
      requestGet(
        admin_url + "document/get_folder_zip/" + id_set + "/" + nameCleaned
      ).done(function (response) {
        response = JSON.parse(response);
      });
      break;
    case "d_file":
      requestGet(admin_url + "document/check_file_exits/" + id_set).done(
        function (response) {
          response = JSON.parse(response);
          if (response.success) {
            requestGet(admin_url + "document/get_file_sheet/" + id_set).done(
              function (response1) {
                response1 = JSON.parse(response1);
  ...
  $('select[name="rel_type[0]"]').on("change", function () {
    var name = $(this).val();
    requestGet(admin_url + "document/get_related/" + _rel_type.val()).done(
      function (response) {
        response = JSON.parse(response);
        $('[for="rel_id[0]"]').html(name);
        if (response == "") {
          $('select[name="rel_id[0]"]').append(
