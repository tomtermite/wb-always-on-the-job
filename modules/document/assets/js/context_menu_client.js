        ...
        switch(moveToAlbumSelectedId) {
          ...
          if(type == 'file'){
            ...
            if(share == false){
              ...
              $.get(site_url + 'document/document_client/check_file_exits/'+id_set).done(function(response) {
                response = JSON.parse(response);
                if(response.success){
                  $.get(site_url + 'document/document_client/get_hash_client/' + id_set).done(function(response1) {
                    response1 = JSON.parse(response1);
                    window.location.replace(site_url + 'document/document_client/file_view_share/'+response1.hash);
                  })
                }else{
          ...
          break;
          case 'delete':
          $.post(admin_url+'document/delete_folder_file/'+id_set).done(function(response){
            response = JSON.parse(response);
          ...
          if(id_set != ''){
            ...
            $.get(admin_url + 'document/get_my_folder/'+ id_set).done(function(response) {
              response = JSON.parse(response);
                ...
                if(response.client_groups_share || response.clients_share){
                  ...
                  $.each(clients_share, function (index, value) {
                    if(index > 0){
                      ...
                      $.get(admin_url + 'document/get_my_folder_get_hash/client/' + value + '/' + id_set).done(function(response_client) {
                ...
                if(response.departments_share || response.staffs_share){
                  ...
                  $.each(staffs_share, function (index, value) {
                    if(index > 0){
                      ...
                      $.get(admin_url + 'document/get_my_folder_get_hash/staff/' + value + '/' + id_set).done(function(response_staff) {
...
$.get(admin_url + 'document/get_folder_zip/'+ id_set + '/' + nameCleaned).done(function(response) {
  response = JSON.parse(response);
})
break;
case "d_file":
$.get(site_url + 'document/document_client/check_file_exits/'+id_set).done(function(response) {
  response = JSON.parse(response);
  if(response.success){
    $.get(admin_url + 'document/document_client/get_file_sheet/'+ id_set).done(function(response1) {
      response1 = JSON.parse(response1);
