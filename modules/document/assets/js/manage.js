(function () {
  ...
  $('select[name="client_groups_share[0]"]').on("change", function () {
    ...
    if (val != "") {
      requestGet(admin_url + "document/append_value_group/" + val).done(
        function (response) {
      ...
      requestGet(admin_url + "document/get_client_all").done(function (
        response_client
      ) {
  ...
  $('select[name="departments_share[0]"]').on("change", function () {
    ...
    var name = $(this).attr("name");
    if (val != "") {
      requestGet(admin_url + "document/append_value_department/" + val).done(
        function (response) {
          response = JSON.parse(response);
      ...
      requestGet(admin_url + "document/get_staff_all").done(function (
        response_staff
      ) {
  ...
  $(".add_share_button").on("click", function () {
    ...
    if (id != "") {
      ...
      requestGet(admin_url + "document/get_my_folder/" + id).done(function (
        response
      ) {
        response = JSON.parse(response);
        ...
        if (response.group_share_client == 2) {
          ...
          hide_old != ""
            ? $('input[name="value-hidden"]').val(
                hide_old + "," + response.group_share_client
              )
            : $('input[name="value-hidden"]').val(response.group_share_client);
          ...
          if (response.client_groups_share || response.clients_share) {
            ...
            $.each(clients_share, function (index, value) {
              ...
              $('select[name="client_groups_share[' + index + ']"]')
                .val(client_groups_share[index])
                .selectpicker("refresh");
              $('select[name="clients_share[' + index + ']"]')
                .val(client_share_value)
                .selectpicker("refresh");
                onchane_handle_client_document(index, client_share_value);
              if (value != "") {
                requestGet(
                  admin_url +
                    "document/get_my_folder_get_hash/client/" +
                    value +
                    "/" +
                    id
        ...
        if (response.group_share_staff == 1) {
          ...
          hide_old != ""
            ? $('input[name="value-hidden"]').val(
                hide_old + "," + response.group_share_staff
              )
            : $('input[name="value-hidden"]').val(response.group_share_staff);
          ...
          if (response.departments_share || response.staffs_share) {
            ...
            $.each(staffs_share, function (index, value) {
              ...
              $('select[name="departments_share[' + index + ']"]')
                .val(departments_share[index])
                .selectpicker("refresh");
              $('select[name="staffs_share[' + index + ']"]')
                .val(staff_share_value)
                .selectpicker("refresh");
                onchane_handle_department_document(index, staff_share_value);
              if (value != "") {
                requestGet(
                  admin_url +
                    "document/get_my_folder_get_hash/staff/" +
                    value +
                    "/" +
                    id
                ).done(function (response_staff) {
                  response_staff = JSON.parse(response_staff);
                  if (response_staff) {
                    $('select[name="role_staff[' + index + ']"]')
                      .val(response_staff.role ? response_staff.role : 1)
                      .selectpicker("refresh");
                  }
                });
              }
  ...
  $("#document-advanced .tr-pointer").on("click", function () {
    ...
    $('#AddFileModal input[name="parent_id"]').val(id);
  });
  ...
  $(document).on("dblclick","#document-advanced .tr-pointer",function() {
    ...
    if(type == "file"){
      if(share == false){
        // requestGet(admin_url + 'document/check_file_exits/'+id_set).done(function(response) {
        //   response = JSON.parse(response);
        //   if(response.success){
        //     window.location.replace(admin_url + 'document/new_file_view/'+parent_id+'/'+id_set);
        //   }else{
        ...
        requestGet(admin_url + 'document/get_hash_staff/' + id_set).done(function(response) {
          response = JSON.parse(response);
          window.location.replace(admin_url + 'document/file_view_share/'+response.hash);
        })
      }
    }else{
  ...
  $(".add_file_button").on("click", function () {
    ...
    // window.location.replace(admin_url + 'document/new_file_view/'+parent_id);
  });
  ...
  //   var name = $(this).val();
  //   requestGet(admin_url + 'document/get_related/' + _rel_type.val()).done(function(response) {
  //     response = JSON.parse(response);
  ...
  $(document).ready(function () {
    $(".share-status").on("click", function () {
      ...
      requestGet(admin_url + "document/get_share_staff_client/" + id_set).done(
        function (response) {
          response = JSON.parse(response);
    ...
    $("body").on('click', '.new_box_information_review', function() {
      ...
      onchane_handle_department_document(addMoreBoxInformationReviewInputKey, staff_share_value);
      addMoreBoxInformationReviewInputKey++;
    });
    ...
    $("body").on('click', '.new_box_information_review_client', function() {
      ...
      onchane_handle_client_document(addMoreBoxInformationReviewInputKey, client_share_value);
  ...
  //     requestGet(admin_url + 'document/get_related_id/'+ val).done(function(response_s) {
  //       response_s = JSON.parse(response_s);
  //       console.log(response_s);
  //       $.each(response_s.type, function(index, value){
  //         requestGet(admin_url + 'document/get_related/' + value + '/' + response_s.id[index]).done(function(response) {
  //           response = JSON.parse(response);
  ...
  if ($("#name").val() != "") {
    $.post(admin_url + "projects/folder/" + project_id, {
      ...
      if ($("#folder_file_id").val() != "" && $("#folder_file_id").val() > 0) {
        ...
        initDataTable(
          ".table-project_file",
          admin_url + "projects/project_files_table/" + project_id
        );
      }
    });
    $("#folder").modal("hide");
  ...
  $('input[type="checkbox"]').click(function () {
    ...
    if (hide_old.length == 3) {
      ...
      if (val == hide_old) {
        ...
        hide_old != ""
          ? $('input[name="value-hidden"]').val(hide_old + "," + val)
...
function onchane_handle_department_document(index, staff_share){
  "use strict";
  $('select[name="departments_share['+index+']"]').on('change', function(){
   var val = $(this).val();
   var name = $(this).attr("name");
   if(val != ''){
     requestGet(admin_url + 'document/append_value_department/'+ val).done(function(response) {
       response = JSON.parse(response);
     ...
     requestGet(admin_url + 'document/get_staff_all').done(function(response_staff) {
       response_staff = JSON.parse(response_staff);
 ...
 function onchane_handle_client_document(index, client_share){
  "use strict"; 
  $('select[name="client_groups_share['+index+']"]').on('change', function(){
   var val = $(this).val();
   var name = $(this).attr("name");  
   if(val != ''){
     requestGet(admin_url + 'document/append_value_group/'+ val).done(function(response) {
       response = JSON.parse(response);
     ...
     requestGet(admin_url + 'document/get_client_all').done(function(response_client) {
       response_client = JSON.parse(response_client);
